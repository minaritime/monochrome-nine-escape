...
async function main(): Promise<void> {
  ...
  // **패치로그** (2026-09-10). 톱니 왼쪽 버튼입니다
  {
    ...
    if (!latest.groups.some((g) => g.items.some((it) => isGatedItem(it)))) {
  ...
  // **여기서도 한 번 더 비웁니다** (2026-09-16). 4번에서 비운 뒤에도 일시정지를
  // 열고 닫는 동안 판이 몇 프레임 더 돌아서, 그 사이에 레벨업이 뜨면 아래
  // "비밀번호를 치면 열린다"의 "오버레이가 비어 있다"가 깨집니다. 잠금 화면이
  // 열린 뒤에는 판이 멈추므로, 여는 순간 전에 비워두는 것이 마지막 기회입니다
  drainChoices();
  press('F1');
  frames(2);
