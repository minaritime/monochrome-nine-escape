...
export class Canvas2DRenderer implements Renderer {
  ...
  constructor(private canvas: HTMLCanvasElement) {
    ...
    this.ctx = ctx;
    this.resize();
    // **여기에 resize 리스너를 달지 마십시오.** 창 크기 변화는 `main.ts` 의
    // `applyLayout()` 하나가 받습니다. 예전에는 양쪽에 달려 있어서 두 번 돌았고,
    // 패널 접기와 순서가 엇갈릴 여지가 있었습니다
  }

  /**
   * 좌우 정보 패널을 화면에서 뺍니다 (모바일).
   *
   * 패널을 빼면 경기장만 남아 1280 x 720, 즉 16:9 가 됩니다.
   * **요즘 폰 가로는 20:9(약 2.16:1)라 이래도 좌우에 여백이 남습니다.** 경기장이
   * 커지는 양은 약 10% 이고, 16:9 폰에서나 "딱 맞는" 이야기였습니다. 그래도 모바일
   * 기본값인 이유는 좁은 화면에서 패널 두 장이 정보를 나르기에 너무 작아서입니다.
   *
