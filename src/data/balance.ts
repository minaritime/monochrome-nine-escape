...
export const ENEMY_BASE = {
   ...
   * 일반 적이 코인을 떨굴 확률.
   *
   * **정예의 `ELITE.coinChance` 와 짝입니다** (2026-09-12). 한쪽만 만지면 판당
   * 코인이 통째로 움직입니다. 정예가 판 전체 코인의 60~70% 를 담당하기 때문입니다
   */
  coinChance: 0.035,
  /** 적끼리 겹치지 않게 밀어내는 힘 */
  separation: 42,
...
export const ENEMY_PARAMS = {
  ...
  /**
   * 소환적은 **잡을 수 있는 하수인**을 부릅니다 (2026-09-09 재설계).
   *
   * 예전에는 무적 · 타겟 불가라 본체를 잡는 것이 유일한 답이었습니다. 지금은
   * 하수인도 평범하게 맞고 죽지만 **잡아도 아무것도 안 나옵니다.** 경험치도
   * 처치 통계도 코인도 없습니다. 5초마다 무한히 다시 채워지는 적이라 보상을
   * 붙이면 소환적 하나가 무한 경험치 공급기가 됩니다.
   *
   * 대신 보상은 전부 **본체 쪽에** 몰아 두었습니다. 본체를 잡는 순간 살아 있던
   * 하수인이 한꺼번에 사라지면서 그 수만큼 코인이 떨어집니다. 하수인을 치우고
   * 다니면 그 몫이 줄어들므로, **모아둔 채 근원을 끊는 쪽이 이득**입니다.
   * "쫓아가서 본체를 잡는다"는 성격이 무적을 뺀 뒤에도 보상으로 유지됩니다.
   *
  ...
  summoner: {
     ...
     * 하수인은 여전히 화면의 적 상한(SPAWN.maxAlive)에 안 세어지므로, 잡을 수
     * 있게 된 대신 5 → 3 으로 줄여 소환적 넷이 화면을 채우지 않게 했습니다
     */
    maxMinions: 3,
    /** 이 셋 중 하나가 스폰 시점에 정해집니다 */
    minionPool: ['fool', 'charger', 'tank'] as readonly EnemyId[],
    /** 본체를 잡을 때 살아 있던 하수인 한 마리마다 떨어지는 코인 수 */
    minionCoinPerAlive: 1,
  },
  /**
  ...
  mummy: {
    ...
    damageMul: 2,
    /**
     * 부활 직후 이동속도. 부활 전의 이 배율입니다 (2026-09-13 에 2.5 → 4.5).
     *
     * 2.5 로는 클리어 전 가장 빠른 경우(하드 15 · 29분, 170)도 상점 0단계 플레이어(205)
     * 보다 느려서, 걸어서 떨어지면 끝나는 적이었습니다. 4.5 면 0단계는 따라잡히고
     * 10단계(285) 쯤부터 떨어질 수 있습니다
     */
    speedMul: 4.5,
    /**
     * 부활 직후 이동속도의 절대 상한 (2026-09-13).
     *
     * **클리어 뒤를 위한 값입니다.** 그 구간에는 급상승 배율(x2.5 ~ x4)이 이미 곱해져
     * 있어서 4.5배면 541 ~ 892 가 되어 사실상 순간이동입니다. 클리어 전 최대가 306 이라
     * 그 구간에서는 안 걸립니다
     */
    revivedSpeedCap: 600,
    /**
     ...
     *
     * **부활한 미라는 `hpDrainPerSec` 로 5초면 스스로 무너집니다.** 그래서 4.5배에서
     * 시작해 무너질 때쯤 4배 언저리입니다. 되살아난 동안은 사실상 계속 빠릅니다
     */
    speedDecayPerSec: 0.1,
    /**
     * 되살아난 뒤 받는 **기절 · 감속의 지속시간** 배율 (2026-09-13).
     *
     * 받는 피해를 80% 줄여 둔 적이라 화력으로는 대응할 길이 없습니다. 대신 묶어 둘 길을
     * 줍니다. 넉백 기절 2초가 4초(수명의 80%), 사슬 작살 1.2초가 2.4초, 체인 마비 0.5초가
     * 1초가 됩니다. 스킬에 딱지를 붙이는 대신 이미 있는 상태이상 축을 쓰므로 새 스킬도
     * 저절로 따라옵니다.
     *
     * **화상은 안 늘립니다.** 보스 상태이상 저항에서 화상을 뺀 것과 같은 이유로, 화상은
     * 상태이상이라기보다 지속 피해입니다. 적용은 `World.stunEnemy` / `slowEnemy` 두 곳뿐입니다
     */
    revivedStatusMul: 2,
    /** 매초 빠져나가는 최대 체력의 비율 */
    hpDrainPerSec: 0.2,
...
/**
 * **클리어한 뒤부터 걸리는 급상승** (2026-09-10).
 *
 * 클리어는 이제 "클리어 시간에 나오는 보스를 잡는 것"이고, 그 보스를 잡는 순간부터
 * 타이머가 다시 흐르며 여기 적힌 값이 `TIME_SCALING` **위에 더해집니다.**
 *
 * 목적은 **낮은 난이도에서 무한히 버티며 파밍하는 것을 막는 것**입니다. 예전에는
 * 클리어 시간을 넘겨도 판이 그대로여서, 난이도 0 에서 도망만 다니면 코인이 계속
 * 들어왔습니다. 지금은 클리어 뒤 몇 분이면 서 있을 자리가 없어집니다.
 *
 * **속도는 시작점이 곧 벽입니다** (2026-09-12 재설계). 예전에는 x1.0 에서 시작해
 * 가속이 붙는 구조였는데, 그러면 도망칠 수 없게 되기까지 4분이 걸려서 그 사이가
 * 통째로 파밍 구간이었습니다. 지금은 **클리어하는 그 순간 이미 `speedStart`** 이고,
 * 정예 빠른적이 467 로 상점만 산 플레이어(365)보다 빠릅니다. 곡선을 가파르게
 * 만드는 대신 출발선을 올린 것이라, 여기서부터는 선형으로만 오릅니다.
 *
 * **체력·공격력은 합연산을 지킵니다.** 곱하면 이미 x7 인 30분 체력에 배율이 다시
 * 얹혀 한 칸에 몇 배씩 뛰고, 특히 공격력을 곱연산으로 두면 접촉 한 번에 즉사가 되어
 * 피할 실력이 아니라 운이 결정하고 체력 강화와 부활이 통째로 죽습니다.
 *
 * **속도에 상한이 있는 이유**는 플레이어 이동속도 상한이 510 이기 때문입니다.
 * x4 면 정예 빠른적이 748 이라 그 위는 같은 뜻의 큰 숫자일 뿐입니다.
 * 적 수 상한도 성능 점검 기준(200마리) 안에서 멈춥니다.
 */
export const OVERTIME = {
  /**
   * **클리어하는 순간의 속도 배율.** 0 분부터 이 값입니다.
   *
   * 이 한 줄이 급상승 구간의 성격을 정합니다. 낮추면 다시 "몇 분은 버틸 만한"
   * 구간이 생기고, 그 몇 분이 그대로 파밍 시간이 됩니다
   */
  speedStart: 2.5,
  /** 분당 속도 증가량 (선형). 3분이면 상한에 닿습니다 */
  speedPerMinute: 0.5,
  /** 속도 배율 상한. 플레이어 이동속도 상한 510 을 한참 넘기는 자리입니다 */
  speedMax: 4.0,
  /** 분당 체력 증가 (합연산) */
  hpPerMinute: 1.5,
  /** 분당 공격력 증가 (합연산) */
  damagePerMinute: 1.5,
  /** 분당 동시 등장 상한 증가 비율 (합연산) */
  maxAlivePerMinute: 0.6,
  /**
   * **클리어 뒤 코인이 떨어질 확률에 곱하는 값** (2026-09-12 사용자 확정).
   *
   * 클리어 뒤 구간은 적 상한이 200 까지 오르므로 분당 처치가 판에서 가장 많아집니다.
   * 그대로 두면 파밍을 막으려고 넣은 급상승이 오히려 파밍 효율을 올립니다. 그렇다고
   * 0 으로 끊으면 적을 잡아도 아무 일도 안 일어나는 구간이 되므로, **눈에 띄게
   * 덜 나오게만** 합니다.
   *
   * **거는 자리는 `World.dropCoin` 첫 줄 하나입니다.** 부르는 쪽(일반 처치 · 정예 ·
   * 보스 · 하수인)에 각각 두면 새 출처가 생겼을 때 반드시 하나를 빠뜨립니다
   */
  coinDropMul: 0.25,
  /** 동시 등장 상한의 끝. `smoke.ts` 성능 점검이 재는 마릿수와 같습니다 */
  maxAliveCap: 200,
} as const;
 ...
 * **클리어 보너스** (2026-09-12).
 *
 * 클리어 보스를 잡는 순간 즉시 적립됩니다. 난이도 코인 배율(`coinMul`)이 그 위에
 * 곱해지므로 난이도 15 면 1,300 입니다.
 *
 * 드랍률을 절반으로 깎으면서 같이 넣은 것입니다. 깎기만 하면 판당 코인이 그대로
 * 반토막인데, 여기에 보너스를 붙이면 **"오래 버티는 것"이 아니라 "빨리 깨는 것"에
 * 돈이 붙습니다.** 클리어 뒤로는 코인이 아예 안 나오므로 이 둘이 짝입니다.
 *
 * **제값은 그 난이도를 처음 깰 때만 나옵니다** (2026-09-12 사용자 확정). 두 번째부터는
 * `repeatRatio` 만큼으로 줄어듭니다. 안 그러면 깨기 쉬운 난이도를 반복해서 도는 것이
 * 가장 효율적인 벌이가 되어, 파밍을 막으려고 넣은 장치가 새 파밍을 만듭니다.
 *
 * 이 규칙 덕분에 **"코인을 더 벌려면 아직 안 깬 난이도로 가라"** 가 성립합니다.
 * 난이도 배율(`coinMul`)도 위로 갈수록 오르므로 둘이 같은 방향을 가리킵니다.
 */
export const CLEAR_BONUS = {
  /** 그 난이도를 **처음** 깼을 때 (난이도 코인 배율을 곱하기 전) */
  coin: 400,
  /**
   * 두 번째부터 남는 비율. 난이도 0 기준 160 코인입니다.
   *
   * 0.2 → **0.4** (2026-09-15 사용자 확정). 클리어 시간을 15분으로 줄이면서 반복 판의
   * 분당 코인이 10~20% 줄어 상점이 느려졌습니다. 판 길이가 전부 17분으로 같고 보너스가
   * 난이도 배율을 따라가므로, 올려도 낮은 난이도 반복이 이득이 되지는 않습니다
   */
  repeatRatio: 0.4,
  /**
   * 첫 클리어 보너스는 배율을 곱한 뒤 이 단위로 반올림합니다 (2026-09-14 사용자 확정).
   * 460 · 1,060 처럼 십의 자리가 붙으면 보상표가 안 읽힙니다. **반복 클리어에는 안 겁니다**
   */
  firstRoundTo: 100,
} as const;
...
export const ELITE = {
   ...
   * 정예를 잡았을 때 코인이 나올 확률.
   *
   * **예전에는 확정 1개였습니다** (2026-09-12 변경). 정예 비율이 10분에 40% 로
   * 고정되므로 확정 드랍이면 정예 한 마리가 일반 적 14마리 몫이 되고, 판 전체
   * 코인의 60~70% 가 여기서 나왔습니다. 판당 코인을 조절하는 가장 큰 손잡이라,
   * 여기를 만질 때는 `ENEMY_BASE.coinChance` 도 같이 보십시오
   */
  coinChance: 0.5,
  /** 표식 색 (링, 뿔, 체력바 공통) */
  markColor: '#ff2d2d',
...
export const SKILLS = {
   ...
   *
   * **`life` 는 반드시 `cooldown` 보다 넉넉히 길어야 합니다** (2026-09-09).
   * 미사일은 겨눈 적이 죽으면 가장 가까운 적을 새로 찾아 틀어지는데, 예전에는
   * 3.2초(쿨의 1.28배)라 **다시 겨누고 도는 동안 다음 발사가 오고 그 사이에
   * 못 맞히면 그냥 사라졌습니다.** 유도한다는 것 자체가 시간을 쓰는 일이라
   * 수명이 쿨과 비슷하면 그 성질이 벌이 됩니다. 지금은 쿨의 두 배입니다.
   *
   * **수명이 다하면 그 자리에서 터집니다** (`onExpire`). 끝까지 못 맞혔다고
   * 아무 일도 없이 사라지면, 유도가 빗나갔을 때의 손해가 통째로 0 이 됩니다.
   * 그 폭발도 **방패는 그대로 존중합니다.** 안 그러면 "미사일은 방패에 막힌다"를
   * 수명이 다할 때까지 기다리는 것으로 우회할 수 있습니다.
   */
  missile: {
    cooldown: 2.5, damage: 0.9, damagePerLevel: 0.095,
    count: 3, countPerLevel: 1, speed: 300, turnRate: 6.0, life: 5.0, blast: 26,
  },
  /**
  ...
  /**
   * 하이리스크 하이리턴 (2026-08-12, 2026-09-09 에 다시 잡았습니다).
   * 쓸 때마다 **최대 체력의 20% 를 스스로 깎고** 그 대가로 주변을 통째로 지웁니다.
   *
   * 예전에는 피해가 없는 것과 같아서(만렙 초당 0.27) 대시를 이길 이유가 없었습니다.
   * 유틸은 한 칸뿐이라 "무적으로 빠져나간다" 하나만 정답이면 나머지가 장식이 됩니다.
   *
   * **2026-09-09: 위력을 통째로 5배로 올렸습니다** (1.65/0.4 → 8.25/2.0).
   * 만렙 2625% 이고 반경 236 안의 적 전부에게 들어갑니다. 5초에 한 번이라 초당으로는
   * 5.25 배수라 공격 스킬(체인 9.4 · 지뢰 10.0 · 미사일 8.4)보다 낮지만, **한 번에
   * 터지는 양은 이 게임에서 가장 큽니다.** 최대 체력의 20% 를 매번 태우는 스킬이
   * 그만한 값을 못 하면 고를 이유가 없다는 판단입니다.
   *
   * **⚠ 같은 날 위험 쪽도 같이 낮아졌습니다.** 체력이 `selfDamageRatio` 이하면
   * 아예 발동하지 않게 막았기 때문에 **이 스킬로는 더 이상 죽지 않습니다.**
   * 두 변경이 같은 방향(안전해지고 세짐)이라, 과하다고 느껴지면 되돌릴 손잡이는
   * 순서대로 `damagePerLevel` → `damage` → `cooldown` 입니다.
   */
  knockback: {
    cooldown: 5.0, damage: 8.25, damagePerLevel: 2.0,
    radius: 152, radiusPerLevel: 9.33, force: 470, stun: 2.0,
    /**
     * 쓸 때마다 **최대 체력**의 이 비율만큼 스스로 깎습니다 (현재 체력 기준이 아닙니다).
     *
     * **이 값이 발동 하한도 겸합니다.** 남은 체력이 최대 체력의 이 비율 이하면
     * 아예 안 나갑니다. 하한을 따로 적으면 대가를 바꿀 때 한쪽만 고쳐서
     * "쓸 수는 있는데 쓰면 죽는" 구간이 다시 생깁니다
     */
    selfDamageRatio: 0.2,
  },
  timeslow: {
...
export interface SkillBranchDef {
  ...
  /**
   * 도탄이 **다음 적 쪽으로 안 틉니다** (관통 도탄, 2026-09-10).
   *
   * 기본 도탄은 맞을 때마다 가장 가까운 다음 적으로 방향을 틀어서 명중 횟수를 거의
   * 확정으로 다 씁니다. 이 갈래는 그 유도를 빼는 대신 훨씬 빠르게 날아갑니다.
   *
   * **벽 튕김은 그대로입니다.** 벽까지 안 튕기면 `직선 + 전관통 + 초고속` 이 되어
   * 레이저(`유일한 전관통`)와 같은 스킬이 됩니다. 벽을 튕기며 계속 살아 돌아다니는
   * 것이 도탄을 도탄으로 만드는 부분이라, 여기를 빼면 갈래가 아니라 다른 스킬입니다
   */
  straight?: boolean;
...
export const SKILL_BRANCHES = {
  ...
  ricochet: [
    ...
    },
    {
      ...
      desc: '적에게 안 틀고 꿰뚫습니다. 훨씬 빠릅니다',
      // **분열을 걷어내고 관통으로 갈아탔습니다** (2026-09-10 사용자 확정).
      //
      // 분열이던 시절에는 갈라진 자식이 `pierce` 를 그대로 복사해서 총 타격이
      // (탄 수) x (명중 횟수) 가 됐고, 4세대면 한 번 발사에 **217타**였습니다.
      // 한 세대로 줄이니 이번에는 반대로 기울어 기본보다도 약해졌습니다.
      // **무엇보다 미사일도 분열이라 갈래 24개 중 둘이 같은 말을 하고 있었습니다.**
      //
      // 지금은 "유도를 버리고 속도를 산다"입니다. 유도가 없어 명중이 확률적이 되는
 ...
 *
 * **칸 봉인은 없앴습니다** (2026-09-13). 봉인하면 한 스탯에 70% 를 통째로 몰 수 있어서
 * 그 스탯만 비정상적으로 커졌습니다. 이제 칸은 늘 셋이고 **한 스탯의 몫은 23.3% 가 끝**입니다.
 * 빈 칸의 몫은 그대로 일반 추첨으로 흘러갑니다.
 *
 * **칸마다 들어갈 수 있는 종류가 정해져 있습니다** (2026-09-13). 주요 칸 하나, 부가 칸 둘입니다.
 * 주요 스탯(공격력 · 이동속도 · 최대 체력 · 연사)을 둘 이상 고정하면 판의 성장이 주요 스탯에
 * 쏠려서, 같은 이유로 과성장이 생깁니다. 판정은 `meta/shop.ts` 의 `togglePassive` 한 곳이고,
 * 옛 저장은 `save.ts` 의 `seatPassives` 가 제 종류 칸으로 다시 앉힙니다.
 */
export type PassiveSlotKind = 'major' | 'minor';

export const PASSIVE = {
  /** 칸 수. 처음부터 전부 열려 있습니다 */
  slots: 3,
  /** 칸마다 들어갈 수 있는 스탯 종류. 길이는 `slots` 와 같아야 합니다 */
  slotKinds: ['major', 'minor', 'minor'] as readonly PassiveSlotKind[],
  /** 지정 칸이 발동할 확률 */
  chance: 0.7,
  /**
   * 패시브 해금 가격 (2026-09-13 에 10배). 봉인이 없어진 대신 하나하나를 무겁게 삽니다.
   * 이미 산 것은 그대로 남고 차액을 받지 않습니다
   */
  majorCost: 4000,
  minorCost: 3000,
} as const;
...
/** 시작 스킬 해금 비용 (2026-09-13 에 전부 2배. 이미 산 것은 그대로입니다) */
...
export const HARD = {
  /**
   * ⚠ **캔버스 덧칠은 없앴습니다** (2026-09-06 사용자 지시).
   *
   * 처음에는 화면 전체를 덮었고, 그다음에는 경기장 배경에만 걸었는데 **둘 다
   * 인게임이 붉어져서** 적과 탄을 읽기 어려웠습니다. 하드모드의 겉모습은 이제
   * **메뉴 배경과 페이지 여백에서만** 냅니다 (`style.css` 의 `body.hard`).
   *
   * **글씨색과 버튼색은 건드리지 않습니다.** 배경 계열 변수만 옮깁니다.
   * 다시 캔버스에 덧칠을 넣지 마십시오.
   */
  /**
   * 하드 0 의 출발 배율.
   *
   * **하드의 출발선은 일반 15 입니다.** 다만 그 배율을 그대로 첫 칸에 얹으면
   * 들어가자마자 불쾌하고 도전할 마음이 꺾입니다. 그래서 **장치는 전부 가져오되
   * 배율만 이만큼으로 줄여서** 시작합니다 (2026-09-06 사용자 확정).
   *
   * **"1 로부터의 거리"에 곱합니다.** 체력 x2.10 이면 1 + 1.10 x 0.7 = x1.77 입니다.
   * 배율 자체에 곱하면 (x1.47) 1 아래로 내려가는 값이 생겨서 뜻이 뒤집힙니다.
   *
   * 이 값은 **초안입니다.** 직접 플레이해서 조정하십시오.
   */
  startMul: 0.7,
  /**
   * 하드 0 의 정예 비율 배율.
   ...
   * **이 값도 초안입니다.** 하드 전량을 60~70판에 채울 수 있는가가 기준입니다.
   */
  coinBase: 3.25,
  coinPerLevel: 0.25,
} as const;
...
/**
 * 하드모드 난이도 표 (2026-09-06).
 *
 * **일반 표(`DIFFICULTY_STEPS`)와 쌓는 방식은 같습니다.** 각 칸에 적힌 것은 그 단계에서
 * 새로 더해지는 양이고, 배율은 곱이 아니라 합으로 쌓입니다.
 *
 * ⚠ **지금은 배율뿐인 뼈대입니다.** 일반 15 의 장치(전원 정예 · 선택지 2장 · 무적
 * 바보적 · 웨이브)가 이미 전부 켜진 채로 시작하므로 **새로 켤 장치가 없습니다.**
 * 여기를 무엇으로 채울지는 아직 안 정했습니다 (docs/기획/난이도.md 의 "아직 안 정한 것" 참고).
...
export const HARD_DIFFICULTY_STEPS: readonly DifficultyStep[] = [
  ...
  { label: '적 공격력 +15% · 적탄 속도 +10%', damageMul: 1.15, bulletSpeedMul: 1.1 },
  { label: '적 체력 +15% · 스폰율 +10%', hpMul: 1.15, spawnRateMul: 1.1 },
  { label: '적 공격력 +10% · 충돌 데미지 +15%', damageMul: 1.1, contactDamageMul: 1.15 },
  { label: '적 체력 +20%', hpMul: 1.2 },
  { label: '적 체력 +15% · 공격력 +15% · 이동속도 +5%', hpMul: 1.15, damageMul: 1.15, speedMul: 1.05 },
];
...
/**
 * 디버그 맵 (2026-09-13, 개발자 모드 전용).
 *
 * 설정 화면에서 들어가는 시험장입니다. 난이도 · 시간 · 스폰을 판 중에 바꾸고 적과
 * 보스와 스킬을 마음대로 꺼냅니다. **기록 · 코인 · 업적 · 도감에 아무것도 안 남깁니다.**
 * 밸런스 수치가 아니라 도구의 눈금이라 게임 결과와 무관합니다
 */
export const DEBUG_MAP = {
  /** 스폰율 배율 눈금. 난이도 배율 위에 곱해집니다 */
  rateMulSteps: [0.25, 0.5, 1, 2, 4, 8],
  /** 한 번에 소환하는 마릿수 눈금 */
  countSteps: [1, 5, 10, 30],
  /** "내 주변" 소환 거리. 붙여서 내면 소환하자마자 부딪혀서 행동을 볼 수 없습니다 */
  nearDistance: 240,
  /** 시간 이동 버튼 (초) */
  timeSteps: [-300, -60, 60, 300],
  /** "클리어 직전" 버튼이 클리어 시간보다 몇 초 앞에 세우는가 */
  clearLead: 10,
  /** 레벨업 버튼 */
  levelSteps: [1, 5],
