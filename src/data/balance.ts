...
export const LEVEL = {
  ...
  /**
   * **처치 경험치에 전부 곱하는 배율** (2026-09-15 사용자 확정).
   *
   * 클리어 시간을 15분으로 줄이자 난이도 0~8 의 판당 스킬 선택이 22회에서 15회로 줄어
   * 빌드가 판 끝에야 겨우 모양을 갖췄습니다. 곡선(`xpToNext`)과 선택 간격은 그대로 두고
   * 들어오는 경험치만 늘려 "스킬을 더 많이 찍게" 했습니다.
   *
   * **모든 난이도와 하드에 똑같이 걸립니다.** 성장 부문은 난이도 표 3단계에만 +50% 를
   * 권했고, 전역이면 0~2 까지 쉬워지고 난이도 15 가 전부 만렙 경계(34회)에 가까워진다는
   * 점을 알고 사용자가 골랐습니다 (`docs/기획/성장.md`). 곱하는 곳은 `World.gainXp` 한 곳입니다
   */
  xpGainMul: 2.25,
  /**
...
export const SKIP_UPGRADE = {
   ...
   * 판당 스킬 선택이 스무 번 안팎 오는데 3회로는 막을 수가 없고, 나머지는
 ...
 *
 * `xpToNext` 가 레벨의 제곱이라 +40% 는 레벨로 약 +18% 입니다. 판당 레벨과 선택
 * 횟수는 클리어 시간과 전역 배율(`LEVEL.xpGainMul`)을 바꿀 때마다 달라지므로 여기
 * 적지 않습니다. 조건별 레벨표는 `docs/기획/성장.md` 에 있습니다.
 *
...
export const CHALLENGE_RULES = {
  ...
  shieldMarch: {
     ...
     *
     * 방패와 돌진을 1:3 으로 나눠 채웁니다. 1 + 3 마리로 시작해 **80초에 양쪽이
     * 동시에 상한(5 · 15)에 닿고**, 남은 10초는 그 밀도를 유지한 채 버팁니다.
     *
     * 클리어가 1분 30초라 판의 대부분이 조여오는 구간이고 마지막에만 잠깐
     * 최대치가 유지됩니다. 끝까지 계속 늘어나게 하려면 이 간격을 5.6 초쯤으로
     * 늘리면 클리어 시점에 상한에 닿습니다
     */
    spawnStep: 5,
    /**
     * 판이 시작될 때 유틸을 하나 고르게 할 것인가 (2026-09-16 사용자 지시).
     *
     * 후보는 **유틸 전부**입니다. 목록을 여기 적지 않는 이유는 `UTILITY_SKILL_IDS`
     * 와 두 벌이 되어 유틸이 늘 때 한쪽만 고치게 되기 때문입니다. 펼치는 일은
     * 그 목록을 이미 들여오고 있는 `progression/skillChoice.ts` 가 합니다.
     *
     * **그 뒤로는 유틸이 선택지에 아예 안 나옵니다.** 유틸은 칸이 하나뿐이라
     * 평소에는 판 내내 갈아탈 수 있는데, 이 판은 시작에 한 번 정하고 끝까지
     * 그것으로 버티는 구조입니다
     */
    ...
    shieldSpeedMul: 0.3,
    /**
     * 방패적 체력 배율. 원문의 "+1000%" 입니다 (2026-09-16 사용자 지시).
     *
     * **스폰할 때 `hpMul` 로 겁니다.** 방패 내구도가 최대 체력에서 계산되므로
     * (`spawnEnemy` 의 `shieldRatio`) 나중에 체력만 곱하면 방패는 안 따라옵니다.
     * 이 판은 방패가 안 깨지니 지금은 표가 안 나지만, 규칙이 바뀌면 바로 어긋납니다
     */
    shieldHpMul: 11,
    ...
    chargerHitsToKill: 2,
    /**
     * 돌진적 접촉 피해 (2026-09-16 사용자 지시).
     *
     * **고정값입니다.** 체력을 평타 기준으로 잡는 것과 달리 이쪽은 플레이어 스펙과
     * 무관하게 못박습니다. 이 판은 맞으면 아픈 정도가 판 내내 같아야 읽힙니다
     */
    chargerDamage: 10,
    /**
  ...
  blackout: {
     ...
     * 한때 -65% 로 좁혔다가 되돌렸습니다 (2026-09-16 사용자 지시).
     * 시야가 이 값을 따라가므로 보이는 범위도 같이 정해집니다
     */
    rangeMul: 0.5,
    /**
    ...
    darkColor: '#05070b',
    /**
     * **겁쟁이는 죽어도 다시 안 채웁니다. 시간표대로만 나옵니다** (2026-09-20 사용자 지시).
     *
     * `cowardSpawnStep` 초마다 `cowardSpawnCount` 마리씩, 판 시작과 동시에 한 번 내고
     * 그 뒤로 같은 간격으로 냅니다. **상한이 없습니다.**
     *
     * 상한 + 즉시 재충전을 뒤집은 자리입니다. 그 방식은 잡아도 곧바로 같은 수로
     * 돌아와서, 잘 잡든 못 잡든 화면의 밀도가 같았습니다. **잡은 만큼 조용해지고
     * 못 잡은 만큼 쌓이게** 하려면 나오는 쪽이 내 처리량과 무관해야 합니다.
     *
     * **묶음이 판 초반에 커집니다** (2026-09-20 사용자 지시). 5초당 3마리로 시작해
     * `cowardSpawnRampTime` 초에 5초당 7마리에 닿고, 그 뒤로는 7마리를 유지합니다.
     * 묶음 크기는 반올림하므로 3 · 3 · 4 · 4 · 4 · 5 · 5 · 5 · 6 · 6 · 6 · 7 로 올라갑니다.
     *
     * 0.6마리/초에서 **1.4마리/초**가 됩니다. 1분까지 58마리, 그 뒤로는 분당 84마리라
     * **따라가야 하는 처리량이 1분을 기점으로 두 배가 넘게 뜁니다.**
     *
     * ⚠ **겁쟁이가 스스로 사라지는 길이 없습니다** (같은 날 돌진 후 사망 삭제).
     * 잡지 않으면 그대로 쌓입니다. 이 값이 곧 "내가 따라가야 하는 처리량"입니다
     */
    cowardSpawnStep: 5,
    cowardSpawnCount: 3,
    cowardSpawnCountMax: 7,
    /** 묶음이 끝 값(`cowardSpawnCountMax`)에 닿는 시각(초). 그 뒤로는 그대로입니다 */
    cowardSpawnRampTime: 60,
    /**
     * **자폭적 동시 상한. 판이 갈수록 늘어납니다** (2026-09-19 사용자 지시).
     *
     * 죽은 만큼 곧바로 다시 채우고, 상한에 닿아 있으면 안 냅니다. 처음 값에서 시작해
     * 클리어 시간에 끝 값에 닿도록 곧게 늘어납니다. **겁쟁이와 규칙이 다릅니다.**
     * 자폭적은 사냥감이 아니라 지뢰밭이라, 밟은 만큼 다시 깔리는 쪽이 맞습니다.
     *
     * ⚠ 초안입니다. 16 은 어둠 속에 고르게 깔리면 시야(경기장의 9.3%) 안에
     * 1.5 마리쯤 걸리는 값입니다
     */
    bomberAliveStart: 6,
    bomberAliveMax: 16,
    /**
     * **적은 시야(= 사거리) 테두리에서 이만큼 더 떨어진 자리에만 나옵니다**
     * (2026-09-19 사용자 지시).
     *
     * 자폭적은 어둠 속 아무 데나, 겁쟁이는 가장자리에서 나오는데 **둘 다 이 조건을
     * 반드시 지킵니다.** 가장자리 근처에 서 있으면 겁쟁이가 눈앞에서 생겨나던 것을
     * 막습니다. 한 걸음에 드러나는 자리에 튀어나오면 "숨어 있던 것을 만났다"가 아니라
     * "눈앞에 생겼다"가 됩니다
     */
    spawnMargin: 80,
    /**
     * **같은 종류끼리 이만큼은 떨어져 나오게 합니다** (2026-09-19 사용자 지시).
     *
     * **되도록이고 반드시는 아닙니다.** 후보 자리를 여러 번 뽑아 이 간격을 넘는 첫
     * 자리를 쓰고, 끝까지 못 찾으면 그중 가장 멀리 떨어진 자리를 씁니다. 상한이 차
     * 가면 자리가 모자라서 반드시로 두면 스폰이 막힙니다.
     *
     * 180 은 자폭 16 마리가 경기장(1280 x 720)에 고르게 깔릴 때의 간격(약 240)보다
     * 조금 좁은 값입니다. 뭉친 지뢰밭이 생기지 않을 만큼만 떨어뜨립니다
     */
    spawnSpacing: 180,
    /**
     * 겁쟁이적의 인내 시간(초). 평소에는 `ENEMY_PARAMS.coward.patienceTime` (30초)입니다.
     *
     * **인내가 끝나는 것이 이 판의 "각성"입니다** (2026-09-19 사용자 지시). 각성 전에는
     * 느리게 떠돌기만 하고 달려들지 않습니다. 잡으러 다닐 사냥감입니다.
     * 각성하면 나를 향해 걸어오다가 **어둠 속에서** 돌진합니다.
     *
     * **20초입니다** (2026-09-19 사용자 지시, 30초에서 줄임)
     */
    cowardPatience: 20,
    /**
     * 각성한 겁쟁이가 걸어오는 속도. **겁쟁이 자신의 이동속도에 곱합니다.**
     *
     * 평소 겁쟁이는 29.6px/s 라 경기장 반대편에서 오는 데 30초가 넘게 걸립니다.
     * 2.5 배면 기본적과 같은 74px/s 이고, 플레이어(205)가 걸어서 따돌릴 수 있습니다
     */
    cowardStalkSpeedMul: 2.5,
    /**
     * **각성한 겁쟁이가 돌진을 시작하는 거리 = 시야 반경 + 이 값** (2026-09-19 사용자 지시).
     *
     * **20 입니다** (2026-09-20 사용자 지시, 40 에서 줄임). 시야 165 기준 185 에서
     * 출발합니다. 거의 시야 테두리까지 걸어 들어온 뒤에 달리므로 돌진이 지나는
     * 거리도 짧습니다.
     *
     * **어둠 속에 있어야 한다는 조건은 없앴습니다** (2026-09-20 사용자 지시).
     * 1차 설계 원문은 "시야에 보이지 않는 한에서 충분히 가까워지면 돌진"이었는데,
     * 그 조건과 이 거리를 함께 걸면 실제로 돌진하는 구간이 **165 ~ 185 의 20px 짜리
     * 띠뿐**이었습니다. 사거리 안에서 각성한 개체는 조건이 영영 거짓이라 걸어오기만
     * 했습니다. 이제는 **거리만 봅니다**
     */
    cowardDashMargin: 20,
    /**
     * **각성한 겁쟁이의 돌진 속도 배수** (2026-09-17 사용자 지시).
     *
     * 어둠에서 튀어나오는 것이 느리면 보고 나서 걸어서 비켜도 되는데, 빠르면 **보는
     * 즉시 움직여야 합니다.** 어둠이 주는 반응 시간의 부족을 속도로 굳히는 값입니다.
     *
     * **5배입니다** (2026-09-19 사용자 지시, 2 → 3 → 5). 240 x 5 = 1200px/s 라
     * 시야 테두리(165)에서 몸에 닿기까지 0.1초 남짓입니다. 피하라고 있는 공격이 아니라
     * **못 잡은 대가**입니다. 달리는 동안은 무적이라 받아칠 수도 없습니다
     * (`challengeInvulnerable`). 잡을 기회는 돌진 전뿐입니다.
     *
     * **이 판에서만입니다.** 일반 게임의 겁쟁이(인내 30초)는 그대로입니다
     */
    cowardEnragedDashMul: 5,
    /**
     * 겁쟁이적의 체력 배율 (2026-09-17 사용자 지시).
     *
     * **정예로 내지는 않습니다.** 한때 정예로 냈다가 되돌렸습니다. 정예는 체력 말고도
     * 인식 사거리 x1.5 · 돌진 속도 x1.5 · 크기 · 공격력이 같이 붙고 표식까지 달려서,
     * 어둠 속에서 붉은 링이 번쩍이는 것이 오히려 위치 알림이 됐습니다.
     * **원하는 것은 "한 대에 안 죽는다" 하나뿐**이라 체력만 정예급(`ELITE.hpMul`)으로
     * 올립니다
     */
    cowardHpMul: 1.5,
    /**
    ...
    igniteSpeedMul: 1,
    /**
     * 이 판에서 **레벨업 추첨에 안 나오는 스탯** (2026-09-16 사용자 지시).
     *
     * 사거리를 뺍니다. 시야가 사거리를 그대로 따라가므로, 사거리가 오르면 판이
     * 진행될수록 어둠이 걷혀서 "안 보이는 판"이라는 전제가 스스로 무너집니다.
     *
     * **이동속도도 뺍니다** (2026-09-19 사용자 지시). 추첨 대신 레벨업마다 따로
     * 확정으로 오릅니다 (`levelMoveSpeedStep`).
     *
     * **체력 재생도 뺍니다** (2026-09-20 사용자 지시). 이 판의 대가는 **못 잡은
     * 겁쟁이에게 받는 체력 피해**인데, 시간이 지나면 저절로 지워지는 자원이 있으면
     * 그 대가가 빚이 아니라 이자가 됩니다. 버티기만 해도 회복되는 쪽이 유리해져서
     * 잡으러 나갈 이유가 약해집니다.
     *
     * 재생은 기본값이 0 이고(`PLAYER_BASE.regen`) 도전 판은 상점 영구 강화가 통째로
     * 벗겨진 저장본으로 열리므로, **추첨에서 빼는 것만으로 이 판에서 완전히 사라집니다.**
     *
     * **이 판에서만입니다.** 일반 게임과 1번의 성장 구조는 그대로입니다
     */
    blockedStats: ['range', 'moveSpeed', 'regen'],
    /**
     * **레벨업마다 이동속도가 확정으로 이만큼 오릅니다** (2026-09-19 사용자 지시).
     * 추첨되는 스탯 2개와 별도입니다.
     *
     * 목표는 **1분 30초에 시작 속도(205)의 1.2배**입니다. 같은 날 2배로 시작했다가
     * 1.2배로 낮췄습니다. 레벨에 걸린 값이라 잘 잡는 사람만 빨라지는데, 2배면 초반에
     * 몇 마리 놓친 사람과의 차이가 너무 벌어져 "느려서 못 잡고, 못 잡아서 맞는"
     * 악순환이 생길 수 있었습니다.
     *
     * 경험치는 **잡은 겁쟁이에게서만** 들어옵니다 (돌진 후 사망 · 자폭적은 경험치 없음).
     * 7 은 "4초에 1마리꼴로 잡은 사람"에 맞춘 값입니다. 겁쟁이 1마리 = 경험치 10.8
     * (3 x 1.6 x 2.25) 이라 90초에 22마리면 누적 236 = Lv7 이고, 6번 올라 205 + 42 = 247
     * 로 1.2배입니다. 평소 추첨 상승폭(8)과 비슷합니다
     */
    levelMoveSpeedStep: 7,
    /**
     * **자폭적의 경험치 배율. 0 이라 어떻게 죽어도 경험치가 없습니다** (2026-09-19 사용자 지시).
     *
     * 이 판의 보상은 겁쟁이를 잡는 것 하나에만 붙습니다. 자폭적은 피해 다닐 위험이지
     * 사냥감이 아닙니다. 잡아도, 스스로 터져도 경험치가 없습니다
     */
    bomberXpMul: 0,
    /**
     * 시작 유틸 선택은 **안 합니다** (2026-09-16 사용자 지시).
     *
     * ⚠ 1번 방패 행진에서 정한 규칙을 여기에 그대로 옮겼던 것이 실수였습니다.
     * **스테이지 규칙은 스테이지마다 따로 정합니다.** 한 곳에서 정한 것을 다른
     * 곳에 자동으로 켜지 마십시오
     */
    startUtilityChoice: false,
    /**
     * **판이 시작될 때 공격 스킬을 하나 고릅니다** (2026-09-20 사용자 지시).
     *
     * 1번의 시작 유틸 선택과 다른 규칙입니다. 후보는 **공격 스킬뿐**이고 장수는 평소
     * 레벨업과 같습니다 (도전은 난이도 0 이라 3장). 어둠 속 겁쟁이를 잡으러 다니는
     * 판인데 시작 수단이 평타뿐이라, 첫 각성(20초)까지 몇 마리를 잡느냐가 그 뒤의
     * 이동속도 성장까지 통째로 정합니다.
     *
     * **그 뒤로는 평소와 같습니다** (2026-09-20 사용자 확정). 유틸도 레벨업 후보에
     * 그대로 나오고 교체도 됩니다. 1번처럼 잠그지 않습니다
     */
    startAttackChoice: true,
    /**
     * **인내가 차가는 겁쟁이의 몸 색** (2026-09-20 사용자 지시).
     *
     * 겁쟁이 고유색(노랑)에서 이 색으로 서서히 옮겨가고, 각성하는 순간 이 색에 닿습니다.
     * 각성은 링으로도 알리지만 그것은 **이미 늦은 뒤**의 표시라, "얼마나 남았는가"를
     * 몸으로 읽을 수 있어야 지금 저것을 잡으러 갈지 판단이 섭니다.
     *
     * ⚠ 적 고유색 규칙(CLAUDE.md 5번)을 어기는 것이 아닙니다. 고유색은 그대로
  ...
  summoner: {
     ...
     * 묶는 것은 이 값뿐입니다. 소환사 한 마리가 하수인을 5번까지 부르므로
     * (아래 `maxMinions`) 무적 하수인은 많아야 40마리, 전부 바보적이면 80마리입니다
     * (2026-09-22, 그 전에는 3마리씩 24마리).
     *
    ...
    killsPerLevel: 2,
    /**
     * **레벨이 이만큼 오를 때마다 필요한 처치 수가 1씩 늡니다** (2026-09-23 사용자 결정).
     *
     * 1~3레벨은 2마리, 4~6레벨은 3마리, 7~9레벨은 4마리입니다. 잡을수록 레벨이 오르고
     * 레벨이 오를수록 빨리 잡는 눈덩이 때문에 후반이 오히려 쉬워졌습니다
     */
    killsPerLevelStep: 3,
    /**
     * **늦게 나오는 소환사는 보호막을 두르고 나옵니다** (2026-09-23 사용자 결정).
     *
     * 체력이 아니라 **횟수**입니다. 피해량과 상관없이 한 번 맞을 때마다 한 겹씩 벗겨지고,
     * 다 벗겨진 뒤에야 체력이 깎입니다. 판이 이 시간(초)만큼 흐를 때마다 한 겹씩 늘어
     * 최대 `barrierMax` 겹입니다 (45초 1겹, 90초 2겹, 135초 3겹).
     *
     * 강해진 뒤 새 소환사를 등장하자마자 녹이는 것을 막습니다. 빠르게 잡힐수록 화면이
     * 비던 것이 후반이 쉬워진 원인이었습니다
     */
    barrierStep: 45,
    barrierMax: 3,
    /**
     * 한 겹이 벗겨진 뒤 다음 겹이 벗겨지지 않는 시간(초). 이 동안 들어오는 피해는 전부 막힙니다.
     * 없으면 오라 · 화염처럼 초당 여러 번 들어오는 피해나 다발 투사체가 세 겹을 한 순간에 벗깁니다.
     * 화상의 지속 피해는 보호막이 남아 있는 동안 막히기만 하고 겹을 벗기지 않습니다
     */
    barrierGuardTime: 0.3,
    /** 보호막 고리 색. 적 고유색이 아니라 스테이지가 얹는 표시입니다 (`cowardRageColor` 와 같은 자리) */
    barrierColor: '#bfe9ff',
    /**
    ...
    startAttackChoice: true,
    /**
     * **소환사 한 마리가 두는 하수인 수** (2026-09-22 사용자 지시). 평소는
     * `ENEMY_PARAMS.summoner.maxMinions`(3)입니다. 바보적은 **부르는 횟수**로 세므로
     * 몸은 `foolSplit` 배만큼 됩니다 (5번 불러 10마리)
     */
    maxMinions: 5,
    /**
     * **하수인 종류** (2026-09-22 사용자 지시). 평소 풀의 탱커를 자폭병으로 바꿨습니다.
     * 무적이라 체력이 뜻 없는 탱커 대신, 스치면 점화되는 자폭병이 "쏘는 자리"를 고르게 합니다
     */
    minionPool: ['fool', 'charger', 'bomber'] as readonly EnemyId[],
    /** 바보적 하수인은 나오자마자 이만큼으로 쪼개져 따로 움직입니다 (2026-09-22 사용자 지시) */
    foolSplit: 2,
    /** 쪼개진 바보적이 부른 자리에서 옆으로 벌어져 나오는 거리 */
    foolSplitGap: 16,
    /**
     * 점화된 자폭병의 이동속도 배율. 2번 암전과 같이 **점화해도 안 빨라집니다**
     * (2026-09-22 사용자 지시). 평소에는 `ENEMY_PARAMS.bomber.igniteSpeedMul` 입니다
     */
    igniteSpeedMul: 1,
    /**
...
export const CHALLENGE_STAGES: readonly ChallengeStageDef[] = [
  {
    id: 'shieldMarch',
    name: '방패 행진',
    // 3분 → 2분 → 1분 30초 (2026-09-16 사용자 지시)
    clearTime: 90,
