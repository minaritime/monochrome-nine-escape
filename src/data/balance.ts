...
export const CHALLENGE_RULES = {
  ...
  blackout: {
    ...
    darkColor: '#05070b',
    /**
     * **초당 스폰 마릿수와 종류 비율** (2026-09-17 사용자 지시).
     *
     * 예전에는 "동시에 몇 마리를 유지한다"였습니다. 죽은 만큼만 다시 채우는 방식이라
    ...
    bomberRatio: 0.3,
    /**
     * 안전장치 상한. 이만큼 살아 있으면 더 안 냅니다.
     *
     * **비율이나 속도를 정하는 값이 아닙니다.** 겁쟁이는 돌진하면 죽고 자폭은 터지면
     * 죽으므로 보통은 여기 안 닿습니다. 닿는다면 플레이어가 아무것도 못 잡고 있다는
     * 뜻이라, 그 이상은 화면만 무거워집니다
     */
    maxAlive: 60,
    /**
     * 겁쟁이적의 인내 시간(초). 평소에는 `ENEMY_PARAMS.coward.patienceTime` (30초)입니다.
     *
     * **30초입니다** (2026-09-17 사용자 지시). 같은 날 5초 → 60초 → 30초로 갔습니다.
     *
     * 5초일 때는 나오자마자 전부 달려들어서, 어둠에 무엇이 숨어 있는가가 아니라
     * 계속 밀려오는 것을 쳐내는 판이었습니다. 60초는 반대로 **겁쟁이가 죽지 않고
     * 상한을 독점해** 자폭적이 화면에서 사라졌습니다 (60초 시점에 겁59 / 자1).
     * 30초는 그 사이입니다. 인내 30 + 수명 5 = **한 마리가 35초를 삽니다**
     */
    cowardPatience: 30,
    /**
     * **인내가 끝난 겁쟁이는 이만큼 뒤에 스스로 죽습니다** (2026-09-17 사용자 지시).
     *
     * 5초 동안 두 배 속도로 달려들고 사라집니다. **처치와 같은 판정이라 경험치가
     * 들어옵니다** (`World.killEnemy`). 내가 못 잡아도 끝이 있는 적으로 만들되,
     * 그 끝을 성과로 쳐주는 것이 이 값의 뜻입니다
     */
    cowardEnragedLife: 5,
    /**
     * **이 시각(초)부터는 살아 있는 모든 겁쟁이의 인내가 즉시 끝납니다**
     * (2026-09-17 사용자 지시).
     *
     * 클리어가 120초이고 인내 뒤 수명이 5초라, 110초에 전부 불을 붙이면 **115초에
     * 화면이 비워집니다.** 마지막 10초는 쌓인 것을 한꺼번에 받아내는 구간입니다.
     * 이 뒤에 나오는 겁쟁이도 나오자마자 달려듭니다
     */
    cowardEnrageAllTime: 110,
    /**
     * **인내가 끝나 달려드는 겁쟁이의 돌진 속도 배수** (2026-09-17 사용자 지시).
     *
     * 이 판은 인내가 5초라 사실상 모든 겁쟁이가 이 상태로 달립니다. 어둠에서 튀어
     * 나오는 것이 느리면 보고 나서 걸어서 비켜도 되는데, 두 배면 **보는 즉시
     * 움직여야 합니다.** 어둠이 주는 반응 시간의 부족을 속도로 굳히는 값입니다.
     *
    ...
    cowardHpMul: 1.5,
    /**
     * **5초마다 내가 서 있던 자리에 작은 폭격이 떨어집니다** (2026-09-17 사용자 지시).
     *
     * 어둠 속에서는 **가만히 서 있는 것이 최선**이 됩니다. 움직이면 안 보이는 적에게
     * 걸어 들어가게 되므로, 자리를 지키고 들어오는 것만 처리하는 쪽이 늘 안전합니다.
     * 그 수를 깨는 장치입니다. 예고를 보고 **한 걸음 옮기면 피합니다.**
     *
     * 값을 정한 근거: 예고 1초 동안 기본 이동속도(205)로 205px 를 가고 반경은 70 이라
     * 여유가 큽니다. 맞히는 것이 목적이 아니라 **자리를 비우게 만드는 것**이 목적이라
     * 하드 5 레이저와 같은 결로 잡았습니다 (`HARD_LASER.telegraph` 주석)
     */
    bombInterval: 5,
    bombTelegraph: 1,
    bombRadius: 70,
    /** 기본 체력 100 기준. 세 번 맞으면 위험한 정도입니다 */
    bombDamage: 24,
    /**
