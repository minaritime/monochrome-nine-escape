...
export const HARD = {
  /**
   * 경기장 **배경에만** 덮는 붉은 막. **은은해야 합니다.**
   *
   * 진하게 하면 적탄(빨강)과 배경이 섞여서, 날아오는 것을 못 봅니다.
   * `scripts/smoke.ts` 가 이 값이 옅은 범위 안에 있는지 잽니다.
   *
   * **화면 전체를 덮으면 안 됩니다** (2026-09-06). 예전에는 render 맨 끝에서
   * 전부 덮었는데 적과 탄과 HUD 까지 같이 흐려져서 화면이 뿌옇고 눈이 아팠습니다.
   * 지금은 경기장 바닥 위, 엔티티 아래에 겁니다 (`scene.ts` 의 `drawWorld`).
   */
  tint: 'rgba(255, 48, 48, 0.10)',
  /**
   * 하드 0 의 출발 배율.
   *
