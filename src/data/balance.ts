 ...
 *
 * **속도만 가속이 붙습니다.** 분당 증가량 자체가 `speedStepMinutes` 마다
 * `speedPerMinuteStep` 씩 커집니다 (0~3분 분당 +0.2 → 3~6분 분당 +0.4 → ...).
 * 체력·공격력은 합연산 그대로인데, 속도가 이미 가파르게 오르므로 그것으로 충분합니다.
 * 셋 다 가속을 붙이면 무엇이 죽였는지 알 수 없게 됩니다.
 *
 * **속도에 상한이 있는 이유**는 플레이어 이동속도 상한이 510 이기 때문입니다.
 * x3 이면 정예 빠른적이 560 이라 이미 도망칠 수 없고, 그 위는 같은 뜻의 큰 숫자일
 * 뿐입니다. 적 수 상한도 성능 점검 기준(200마리) 안에서 멈춥니다.
 */
export const OVERTIME = {
  /** 분당 속도 증가량이 이 시간(분)마다 한 단계 커집니다 */
  speedStepMinutes: 3,
  /** 한 단계당 "분당 증가량" */
  speedPerMinuteStep: 0.2,
  /** 속도 배율 상한. 플레이어 이동속도 상한 510 을 넘기는 자리입니다 */
  speedMax: 3.0,
  /** 분당 체력 증가 (합연산). 30초마다 +20% 와 같은 값입니다 */
  hpPerMinute: 0.4,
  /** 분당 공격력 증가 (합연산) */
  damagePerMinute: 0.4,
  /** 분당 동시 등장 상한 증가 비율 (합연산) */
  maxAlivePerMinute: 0.2,
