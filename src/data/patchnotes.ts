 ...
 *
 * **`+ ` 는 플레이어에게 버프(초록), `- ` 는 너프(빨강)입니다.** v0.2.02 부터는 모든
 * 줄에 둘 중 하나를 붙입니다. 부호가 없는 줄은 그 전 버전들이라 색 없이 `- ` 만 붙여
 * 그립니다. 그때는 `-` 가 너프라는 뜻이 아니었으므로 빨강으로 칠하면 거짓말이 됩니다
 */
export type PatchTone = 'buff' | 'nerf' | 'plain';
