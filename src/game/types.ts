...
export interface Enemy {
   ...
   * 몇 번 맞았는가 (2026-09-16).
   *
   * **연속 타격과 화상은 안 셉니다.** 오라나 화염방사기처럼 초당 여러 번 들어오는
   * 것을 세면 한 번 스치는 것만으로 한계에 닿습니다 (`DamageOptions.kind`).
   * 지금 이 값을 보는 것은 도전 1번의 돌진적뿐이고, 그 판에서는 피해량과 무관하게
   * 정해진 횟수를 맞으면 죽습니다
   */
  hits: number;
  /**
...
export interface Telegraph {
  ...
  owner: number;
  /**
   * **어둠에서 시야와 겹치는 부분만 그리는가** (도전 2번 암전).
   *
   * 평소에는 거짓이라 중심이 시야 밖이면 통째로 안 그려집니다. **자폭의 터지는
   * 순간만** 참으로 켭니다 (`World.blastVisual`).
   *
   * 2026-09-16 에는 이 값이 "어둠을 통째로 뚫는다"였습니다. 2026-09-17 에 사용자가
   * **"중심이 밖이어도 사거리 안에 걸치는 만큼은 보이게"** 로 바꿨습니다. 전부
   * 보이면 폭발이 어둠을 걷어내는 조명이 되고, 전부 가리면 바로 옆에서 터진 것도
   * 안 보입니다. 겹치는 만큼만 보이는 것이 둘 사이입니다.
   *
   * 잡았을 때 뜨는 사전 예고 원은 그대로 묻힙니다
   */
  clipToVision: boolean;
  dead: boolean;
}
