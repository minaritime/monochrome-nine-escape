...
export class World {
  ...
  /**
   * 도전 스테이지의 **시작 유틸을 이미 골랐는가** (2026-09-16).
   *
   * 판이 시작되면 선택창이 한 번 뜨고 거기서 고른 유틸로 끝까지 갑니다.
   * 이 값이 참이 되면 그 뒤의 선택지에는 유틸이 아예 안 나옵니다
   */
  challengeStartPicked = false;

  /**
   * 도전 스테이지가 쓰는 작은 상태 묶음 (2026-09-17).
   *
   * 둘 다 **지금까지 몇 번 있었는가**를 셉니다. 흘러간 시간에서 횟수를 다시 계산해
   * 모자란 만큼 처리하는 방식이라 프레임 간격에 기대지 않고, 디버그 맵에서 시간을
   * 건너뛰어도 그 사이의 몫이 사라지지 않습니다. 무적 바보적이 같은 방식입니다
   * (`immortalFoolsSpawned`).
   *
   * 스테이지마다 뜻이 다릅니다. 지금 쓰는 것은 2번 암전뿐입니다
   */
  challengeState = { bombs: 0, spawned: 0 };
...
/**
  ...
  timeScale(): { hp: number; dmg: number; speed: number } {
    // **도전 판은 시간이 지나도 적이 안 강해집니다** (2026-09-16 사용자 지시).
    //
    // 도전은 난이도 배율도 안 붙는 판이라 시간 배율만 붙어 있으면 앞뒤가 안 맞습니다.
    // 난도는 스테이지 규칙과 마릿수로만 올라갑니다. 9번 "개판"처럼 30분짜리
    // 스테이지에서 체력이 저절로 +6.0 배가 되던 것도 이것으로 없어집니다
    if (this.challenge) return { hp: 1, dmg: 1, speed: 1 };
  ...
  killEnemy(e: Enemy): void {
    ...
    if (e.boss) {
      ...
      // 개체마다 정예 드랍을 주면 한 마리에서 코인이 몸통 수만큼 쏟아집니다.
      // 보상은 "정예 한 마리를 잡았다"에 붙는 것이지 몸통 수에 붙는 것이 아닙니다.
      //
      // 전원이 정예인 난이도(9 이상)에서는 이 정예 드랍 분기를 끄고 일반 적 확률로 떨어집니다.
      // 전부 정예인데 전부 정예 확률로 주면 난이도 배율과 겹쳐 코인이 폭증합니다.
      // ⚠ 그 대신 난이도 8 이 9~15 보다 판당 코인을 많이 버는 절벽이 생겨 있습니다
      // (하드 5 → 6 도 같음). 다음 경제 작업 첫 순위입니다 (`docs/기획/경제.md`)
